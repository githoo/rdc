...
from rdc_summary.rdc_data.data_loader import *
...
predict_out_dir = os.path.join(base_dir, 'tcn_predict_out_result_prob.txt')
...
def train():
    ...
    x_train, y_train = process_file(train_dir, word_to_id, cat_to_id, config.seq_length,is_char,0)
    x_val, y_val = process_file(val_dir, word_to_id, cat_to_id, config.seq_length,is_char,0)
...
def test():
    ...
    start_time = time.time()
    x_test, y_test = process_file(test_dir, word_to_id, cat_to_id, config.seq_length,is_char,0)
...
def predict():
    ...
    start_time = time.time()    
    x_predict, _ = process_file(predict_in_dir, word_to_id, cat_to_id, config.seq_length,is_char,1)
